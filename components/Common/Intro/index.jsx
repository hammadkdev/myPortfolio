...
import { FaDownload, FaFacebook, FaGithub, FaLinkedin, FaTwitter, FaRegWindowClose } from 'react-icons/fa';
  ...
  return (
    <>
      ...
      <div className='headerr z-50 absolute bg-MidNightBlack backdrop-blur-sm inset-y-0 h-48 top-0 flex items-center justify-center w-full flex-col px-4 gap-y-4'>
        <Image className='w-20 h-20 rounded-full' src={Me} alt='profile picture' />
        ...
        <div onClick={() => setIsOpen(val => !val)} className='md:hidden absolute top-2 left-2 text-[30px] text-green-400'>
          <FaRegWindowClose />
        </div>
      </div>
